...
def parse_boarding_pass(bp: str) -> int:
    rows = bp[0:7].replace("F", "0").replace("B", "1")
    cols = bp[7:10].replace("L", "0").replace("R", "1")

    return int(rows, base=2) * 8 + int(cols, base=2)


assert parse_boarding_pass("FBFBBFFRLR") == 357
...
print("Part2:", find_my_seat(seats))
